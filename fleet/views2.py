...
class DashView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['all_units'] = base_units.order_by('body_no')
...
class UnitDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['materials_used'] = Outgoing.objects.filter(unit=self.object).filter(trans_type='Outgoing').order_by('-trans_date')
...
def TravelStatus(request, pk):
    ...
    if travel_status.status == "Scheduled":
        travel_status.status = "In-Transit"
    elif travel_status.status == "In-Transit":
        travel_status.status = "Arrived"
    elif travel_status.status == "Arrived":
        travel_status.status = "Canceled"
    elif travel_status.status == "Canceled":
        travel_status.status = "Scheduled"
